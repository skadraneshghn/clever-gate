...
async def record_failure(
    ...
    try:
        ...
        logger.warning("breaker.record_failure_failed", error=str(exc), deployment_id=deployment_id)
        return 0
...
async def record_success(deployment_id: str) -> None:
    ...
    try:
        ...
        logger.warning("breaker.record_success_failed", error=str(exc), deployment_id=deployment_id)
...
async def is_available(deployment_id: str, max_fails: int = 3) -> bool:
    ...
    try:
        ...
        logger.warning("breaker.check_availability_failed", error=str(exc), deployment_id=deployment_id)
        return True
...
async def get_state(
    ...
    try:
        ...
        logger.warning("breaker.get_state_failed", error=str(exc), deployment_id=deployment_id)
        return {
            "fail_count": 0,
